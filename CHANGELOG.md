...
| 版本 | Commit | 主要內容 |
|------|--------|---------|
| v1.4.0 | (pending) | **GAMLSS 連續常模**：全部 7 項指標（含 PAF）改用 μ(age)、σ(age) 連續函數；年齡改為小數年齡（日精度）；移除 COH √T×10 後處理 |
| v1.3.0 | (pending) | 常模校正（EC 狀態）：APR 6–18 歲、FAA 年齡分層、TBR 6–12、成人 TBR、COH F-P 限縮為 3 對×5 帶 |
...
**背景：** v1.3.0 的常模為 2–6 段階梯式（依整數年齡選 μ/σ），在年齡切換點（6 歲、13 歲、19 歲）會造成 T 分數不連續的跳動。臨床與發育研究顯示腦電指標隨年齡呈平滑變化，本次改用 GAMLSS（Generalized Additive Models for Location, Scale and Shape）擬合的連續 μ(age)、σ(age) 函數。

**核心改動：**

1. **`crate/src/eeg_analysis.rs`** — 7 項指標常模函數由分段常數改為連續公式：

| 指標 | μ(age) | σ(age) |
|------|--------|--------|
| TBR | $4.45 - 0.16a + 0.002a^2$（a = min(age, 40)） | $0.60 + 0.65\,e^{-0.02(a-8)^2}$ |
| APR | $0.15 + 0.15\cdot\tfrac{a}{28}\,e^{1-a/28}$ | $0.075 + 0.025e^{-0.1 a} + 0.025e^{-0.01(a-27)^2}$ |
| FAA | $0$ | $0.05 + 0.05/(1 + e^{a-12.5})$ |
| PAF | $10.2 - 4.59\,e^{-0.095 a}$ | $0.75 + 0.25/(1 + e^{-0.5(a-19)})$ |
| RSA | $8 + 17e^{-0.02(a-3)^2} + 11e^{-0.03(a-27)^2} + 26e^{-0.003(a-72)^2}$ | $\max(\mu - 1, 0.5)$ |
| COH | $0.182\,\ln(a) + 0.15$（a = clip(age,1,25)） | $0.15$（常數） |
| EnTP | $0.48\,a^{0.41}$（a = min(age,20)） | $0.18 + 0.02 a$ |

2. **年齡精度** — `ageFromDob()` 由「整數年齡」改為「日精度小數年齡」（毫秒差 / 365.2425 天）。
3. **COH T 分數統一公式** — 移除先前的 √T × 10 後處理，改用標準 T = 10·Z + 50，與其他 6 項指標一致。
4. **WASM 介面** — `analyze_eeg(samples, age)` 的 `age` 參數由 `u32` 改為 `f64`。
5. **Capability 維度分組** — 仍以整數年齡（`floor(age)`）選擇 4–6 / 7–24 / 25–64 / 65+ 族群。

**影響：** 原本在年齡切換點（例如 12.9 → 13.0 歲）會突然跳動的 T 分數，現改為平滑連續變化。所有受測者的 T 分數會微幅調整以符合 GAMLSS 模型的預測曲線。
